...
### set parameter.
jobdir <- "/data/home/szu/Lab/ml-2dqsar/cv_sigma2dot_sigma2jsame/result/"
datadir <- "/data/home/szu/Lab/ml-2dqsar/"
ml2dqsar_dir <- "/data/home/szu/Lab/ml-2dqsar/cv_sigma2dot_sigma2jsame/"
...
sigma2j_rela <- as.numeric(args[4]) # sigmaj size
sigma2dot_rela <- as.numeric(args[5]) # sigma2dot relative size.
...
for(f in 1:length(features)){# f for feature
  for(k in 1:length(subdirs)){ # k for subdirs.
    ...
    sigma2j <- round(sigma2j_rela*mean(colVars(cpi$omega_matrix)),3)
    sigma2dot <- sigma2dot_rela*sigma2j
    cpi$sigma2j <- sigma2j
    cpi$sigma2dot <- sigma2dot
    for(i in 1:length(cpi$y)){
      cpi$X[[i]] = t(cpi$X[[i]])
    }
    ...
    save(mresult, file=paste('mresult_',subdirs[k],'_',features[f],
                             '_', sigma2j,
                             '_', sigma2dot, sep=""))
  }
}
